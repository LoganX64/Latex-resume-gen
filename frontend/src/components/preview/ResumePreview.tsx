...
import { faTriangleExclamation, faExpand, faMagnifyingGlassPlus, faMagnifyingGlassMinus, faXmark } from "@/lib/icons";
import {
  Select,
  SelectTrigger,
  SelectValue,
...
export function ResumePreview({
  ...
  const [overrideZoom, setOverrideZoom] = useState<ZoomLevel | null>(
    initialZoom ?? null,
  );
  ...
  const pageOffsets = useMemo(() => {
    ...
    if (sectionBounds.length === 0) {
      const count = Math.max(
        1,
        Math.ceil(totalContentHeight / contentAreaHeight),
      );
    ...
    while (currentOffset + contentAreaHeight < totalContentHeight) {
      ...
      for (const section of sectionBounds) {
        ...
        if (
          sectionStart < pageEnd &&
          sectionEnd > pageEnd &&
          section.height <= contentAreaHeight
        ) {
  ...
  const measureContent = useCallback(() => {
    Sentry.startSpan(
      { name: "Measure Resume Content", op: "dom.measure" },
      (span) => {
  ...
  const totalScaledHeight =
    numberOfPages * A4_HEIGHT_PX + (numberOfPages - 1) * 16;
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      const fs = Math.min(
        Math.max((el.clientWidth - 32) / A4_WIDTH_PX, 0),
        Math.max((el.clientHeight - 32) / tsh, 0),
        1.5,
      );
      setFitScale(fs || 1);
    };
  ...
  const baseScale = isFullscreen
    ? zoom === "fit"
      ? Math.min(
          (viewport.w - 40) / A4_WIDTH_PX,
          (viewport.h - 40) / totalScaledHeight,
          1.5,
        )
      : scale
    : zoom === "fit"
    ...
    return (
      <div
        ...
        style={{
          ...
          height: A4_HEIGHT_PX,
          boxShadow:
            "0 15px 35px -10px rgba(0, 0, 0, 0.35), 0 0 0 1px rgba(0, 0, 0, 0.08)",
          overflow: "hidden",
        }}
      >
        <div
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-b border-border backdrop-blur">
            ...
            <div className="flex items-center gap-0.5">
              <Button
                ...
                aria-label="Zoom out"
              >
                <Icon icon={faMagnifyingGlassMinus} className="h-3 w-3" />
              </Button>
              <Select
                value={String(zoom)}
                onValueChange={(v) =>
                  setZoom(v === "fit" ? "fit" : (Number(v) as ZoomLevel))
                }
              >
                <SelectTrigger
                  className="text-[9px] sm:text-[10px] h-6 px-1.5 py-0.5 gap-1 cursor-pointer min-w-0"
                ...
                <SelectContent className="min-w-16 text-[9px] sm:text-[10px]">
                  {zoomLevels.map((z) => (
                    <SelectItem
                      key={z}
                      value={String(z)}
                      className="py-0.5 pr-6 pl-1.5 text-[9px] sm:text-[10px]"
                    >
                      {z}%
                    </SelectItem>
                  ))}
              ...
              <Button
                ...
                aria-label="Zoom in"
              >
                <Icon icon={faMagnifyingGlassPlus} className="h-3 w-3" />
              </Button>
              <Button
                variant="ghost"
                size="icon-xs"
              ...
              >
                <Icon icon={faExpand} className="h-3 w-3" />
              </Button>
            </div>
          </div>
        )}
