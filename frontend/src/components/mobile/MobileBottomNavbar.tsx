...
export function MobileBottomNavbar({
  ...
  return (
    <nav className="flex items-stretch border-t border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 shrink-0 safe-area-inset-bottom">
      {([
        { label: 'Home', icon: <Home className="h-5 w-5" />, onClick: onHome, key: 'home' },
        { label: 'Saved', icon: <Bookmark className="h-5 w-5" />, onClick: onSaved, key: 'saved' },
        { label: 'Preview', icon: <Eye className="h-5 w-5" />, onClick: onPreview, key: 'preview' },
      ] as const).map((item) => (
        <button
          key={item.key}
