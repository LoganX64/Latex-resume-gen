...
const config: TemplateConfig = {
  id: 'sidebar',
  name: 'Modern Sidebar',
  description: 'Two-column layout with a colored sidebar for skills and contact info. Modern and visually appealing.',
  supportsPhoto: true,
  category: 'modern',
}
...
function Preview({
  ...
  return (
    <div className="flex min-h-full" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif', fontSize: '10px', lineHeight: '1.4' }}>
      {/* Sidebar */}
      <div className="w-[140px] shrink-0 text-white p-3" style={{ backgroundColor: '#1a365d' }}>
        <div className="text-center mb-3">
          ...
          <h1 className="text-sm font-bold">{personalInfo.fullName || 'Your Name'}</h1>
          {personalInfo.professionalTitle && (
            <p className="text-[8px] text-white/80 italic mt-0.5">{personalInfo.professionalTitle}</p>
          )}
        </div>
  ...
  switch (section.type) {
    case 'skills':
      return resume.skills.length > 0 ? (
        <div className="mb-2">
          <h3 className="text-[9px] font-bold uppercase tracking-wider mb-1 pb-0.5 border-b border-white/30">
            Skills
          </h3>
          <div className="space-y-1">
            {resume.skills.map((cat) => (
              <div key={cat.id}>
                <span className="text-[7px] font-semibold">{cat.name || 'Category'}</span>
                <p className="text-[7px] text-white/80">{cat.skills.join(', ')}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-2">
          ...
          <div className="space-y-0.5">
            {resume.languages.map((lang) => (
              <div key={lang.id} className="flex justify-between text-[7px]">
        ...
        <div className="mb-2">
          ...
          <div className="space-y-0.5">
            {resume.certifications.map((cert) => (
              <div key={cert.id} className="text-[7px]">
  ...
  switch (section.type) {
    case 'summary':
      return resume.summary ? (
        <div className="mb-2">
          <h3 className="text-[10px] font-bold uppercase tracking-wider mb-1 pb-0.5" style={{ color: '#1a365d', borderBottom: '1px solid #1a365d' }}>
        ...
        <div className="mb-2">
          ...
          {resume.experience.map((exp) => (
            <div key={exp.id} className="mb-2 last:mb-0">
              ...
              <div className="flex justify-between items-baseline">
                <span className="text-[8px] text-gray-600 italic">{exp.company || 'Company'}</span>
                {exp.location && (
                  <span className="text-[7px] text-gray-500">{exp.location}</span>
                )}
              </div>
              {exp.bulletPoints.filter(Boolean).length > 0 && (
                <ul className="mt-0.5 space-y-0.5">
                  {exp.bulletPoints.filter(Boolean).map((bullet, i) => (
                    <li key={i} className="text-[7px] text-gray-700 pl-2 relative before:content-['•'] before:absolute before:left-0" style={{ color: '#1a365d' }}>
                      {bullet}
                    </li>
                  ))}
                </ul>
        ...
        <div className="mb-2">
          ...
          {resume.projects.map((proj) => (
            <div key={proj.id} className="mb-2 last:mb-0">
              ...
              {proj.role && (
                <span className="text-[7px] text-gray-600 italic">{proj.role}</span>
              )}
              {proj.description && (
        ...
        <div className="mb-2">
          ...
          {resume.education.map((edu) => (
            <div key={edu.id} className="mb-1.5 last:mb-0">
              ...
              <div className="flex justify-between items-baseline">
                <span className="text-[8px] text-gray-600 italic">{edu.institution || 'Institution'}</span>
        ...
        <div className="mb-2">
          ...
          {resume.achievements.map((ach) => (
            <div key={ach.id} className="mb-1 last:mb-0">
              <div className="flex justify-between items-baseline">
                <span className="font-semibold text-[8px]">{ach.title || 'Achievement'}</span>
                {ach.date && <span className="text-[7px] text-gray-500">{ach.date}</span>}
              </div>
              {ach.description && (
                <p className="text-[7px] text-gray-700">{ach.description}</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mb-2">
          ...
          {resume.publications.map((pub) => (
            <div key={pub.id} className="mb-1 last:mb-0">
              <div className="flex justify-between items-baseline">
                <span className="font-semibold text-[8px]">{pub.title || 'Publication'}</span>
                <span className="text-[7px] text-gray-500">{pub.date}</span>
              </div>
              <span className="text-[7px] text-gray-600 italic">{pub.publisher}</span>
              {pub.description && (
                <p className="text-[7px] text-gray-700">{pub.description}</p>
              )}
            </div>
          ))}
        </div>
