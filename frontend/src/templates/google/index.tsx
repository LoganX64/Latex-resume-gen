...
function Preview({
  ...
  return (
    <div style={{ fontFamily: '"Roboto", "Helvetica Neue", Arial, sans-serif', fontSize: '10px', lineHeight: '1.4' }}>
      {personalInfo.fullName && (
        <header className="text-center border-b-2 pb-2 mb-2" style={{ borderColor: '#1a73e8' }}>
              ...
              {exp.bulletPoints.filter(Boolean).length > 0 && (
                <ul className="mt-0.5 space-y-0.5">
                  {exp.bulletPoints.filter(Boolean).map((bullet, i) => (
                    <li key={i} className="text-[8px] text-gray-700 pl-2 relative before:content-['•'] before:absolute before:left-0" style={{ color: '#1a73e8' }}>
                      {bullet}
                    </li>
                  ))}
                </ul>
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mb-2">
      <h2 className="text-[10px] font-bold uppercase tracking-wider pb-0.5 mb-1" style={{ color: '#1a73e8', borderBottom: '2px solid #1a73e8' }}>
        {title}
      </h2>
      {children}
    </div>
