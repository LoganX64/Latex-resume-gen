...
export function generateFaangLatex(
  resume: ResumeData,
  ...
  return buildFaangDocument(resume.personalInfo, body)
}

function buildFaangDocument(
  personalInfo: ResumeData['personalInfo'],
  body: string
): string {
  const name = escapeLatex(personalInfo.fullName || 'Your Name')
  const title = personalInfo.professionalTitle
    ? `\\\\[1pt]{\\large\\textit{${escapeLatex(personalInfo.professionalTitle)}}}`
    : ''
