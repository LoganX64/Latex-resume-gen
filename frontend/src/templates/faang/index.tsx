...
import { generateFaangLatex } from './latex'

const config: TemplateConfig = {
  id: 'faang',
  name: 'FAANG Style',
  description: 'Clean, impact-focused layout used by top tech companies. Emphasizes metrics and achievements.',
  supportsPhoto: false,
  category: 'modern',
}
...
function Preview({
  ...
  return (
    <div style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif', fontSize: '10px', lineHeight: '1.4' }}>
      {/* Header */}
      {personalInfo.fullName && (
        <header className="text-center border-b border-gray-300 pb-2 mb-2">
          ...
          <div className="flex flex-wrap justify-center gap-x-2 gap-y-0.5 mt-1 text-[8px] text-gray-600">
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mb-2">
      <h2 className="text-[10px] font-bold uppercase tracking-wider text-black border-b border-gray-300 pb-0.5 mb-1">
        {title}
      </h2>
      {children}
    </div>
...
export default {
  config,
  Preview,
  generateLatex: generateFaangLatex,
}
