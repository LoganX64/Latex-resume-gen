  ...
  return (
    ...
    >
      ...
      {personalInfo.fullName && (
        <header className="text-center border-b border-gray-300 pb-0.5 mb-0.5">
          <h1 className="text-[19px] font-bold tracking-wide text-gray-900">
            {personalInfo.fullName}
          </h1>
          {personalInfo.professionalTitle && (
            <p className="text-[16px] text-gray-900 italic mt-0">
              {personalInfo.professionalTitle}
            </p>
          )}
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-0 mt-1 text-[11px] text-gray-900">
            {personalInfo.email && (
              <span className="inline-flex items-center gap-1">
                <ContactIcon type="email" className="w-3 h-3" />
                {personalInfo.email}
              </span>
            )}
            {personalInfo.phone && (
              <span className="inline-flex items-center gap-1">
                <ContactIcon type="phone" className="w-3 h-3" />
                {personalInfo.phone}
              </span>
            )}
            {personalInfo.location && (
              <span className="italic inline-flex items-center gap-1">
                <ContactIcon type="location" className="w-3 h-3" />
                {personalInfo.location}
              </span>
            )}
            {personalInfo.linkedin && (
              <span className="text-blue-700 inline-flex items-center gap-1">
                <ContactIcon type="linkedin" className="w-3 h-3" />
                {personalInfo.linkedin}
              </span>
            )}
            {personalInfo.github && (
              <span className="text-blue-700 inline-flex items-center gap-1">
                <ContactIcon type="github" className="w-3 h-3" />
                {personalInfo.github}
              </span>
            )}
            {personalInfo.website && (
              <span className="text-blue-700 inline-flex items-center gap-1">
                <ContactIcon type="website" className="w-3 h-3" />
                {personalInfo.website}
              </span>
            )}
          </div>
      ...
      return resume.experience.length > 0 ? (
        <Section title="Work Experience">
          {resume.experience.map((exp) => (
            <div key={exp.id} className="mb-0.5 last:mb-0">
              ...
              <div className="flex justify-between items-baseline">
                <span className="text-[11px] text-gray-900 italic">
                  {exp.company || "Company"}
                </span>
                {exp.location && (
                  <span className="text-[11px] text-gray-800">
              ...
              {exp.bulletPoints.filter(Boolean).length > 0 && (
                <ul className="mt-0 space-y-0">
                  {exp.bulletPoints.filter(Boolean).map((bullet, i) => (
                    <li
                      key={i}
                      className="text-[11px] text-gray-900 pl-2 relative before:content-['•'] before:absolute before:left-0 before:text-gray-600"
                    >
                      {bullet}
                    </li>
                  ))}
      ...
      return resume.projects.length > 0 ? (
        <Section title="Projects">
          {resume.projects.map((proj) => (
            <div key={proj.id} className="mb-0.5 last:mb-0">
              <div className="flex justify-between items-baseline">
                <div>
                  <span className="font-semibold text-[11px]">
                    {proj.name || "Project"}
              ...
              {proj.description && (
                <p className="text-[11px] text-gray-900 mt-0">
                  {proj.description}
                </p>
              )}
              ...
              {proj.technologies.length > 0 && (
                <p className="text-[11px] text-gray-800 mt-px">
                ...
                <p className="text-[11px] text-gray-800 mt-px">
                  {proj.githubUrl && (
                    <span>
                      ...
                      <span className="text-blue-700">{proj.githubUrl}</span>
                    </span>
                  )}
                  ...
                  {proj.liveDemoUrl && (
                    <span>
                      ...
                      <span className="text-blue-700">{proj.liveDemoUrl}</span>
                    </span>
                  )}
                </p>
              )}
      ...
      return resume.certifications.length > 0 ? (
        <Section title="Certifications">
          {resume.certifications.map((cert) => (
            <div
              key={cert.id}
              className="mb-0.5 last:mb-0 flex justify-between items-baseline"
            >
          ...
          {resume.achievements.map((ach) => (
            <div key={ach.id} className="mb-0.5 last:mb-0">
              <div className="flex justify-between items-baseline">
                ...
                {ach.date && (
                  <span className="text-[11px] text-gray-800">{ach.date}</span>
                )}
              </div>
              {ach.description && (
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mb-0.75" data-section="true">
      <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-300 pb-0.75 mb-px">
        {title}
      </h2>
      {children}
    </div>
