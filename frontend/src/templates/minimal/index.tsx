...
function Preview({
  ...
  return (
    <div style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif', fontSize: '13px', lineHeight: '1.2' }}>
      {/* Header */}
      {personalInfo.fullName && (
        <header className="text-center pb-1.5 mb-1.5">
          ...
          {personalInfo.professionalTitle && (
            <p className="text-[13px] text-gray-500 italic mt-0.5">{personalInfo.professionalTitle}</p>
          )}
          <div className="flex flex-wrap justify-center gap-x-2 gap-y-0 mt-1 text-[11px] text-gray-600">
    ...
    case 'summary':
      return resume.summary ? (
        <Section title="Summary">
          <p className="text-[13px] leading-relaxed text-gray-700">{resume.summary}</p>
        </Section>
      ) : null
        ...
        <Section title="Skills">
          <div className="space-y-0.5">
            {resume.skills.map((cat) => (
              <div key={cat.id}>
                <span className="font-bold text-[11px]">{cat.name || 'Category'}: </span>
                <span className="text-[11px] text-gray-700">{cat.skills.join(', ')}</span>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="Education">
          {resume.education.map((edu) => (
            <div key={edu.id} className="mb-1 last:mb-0">
              ...
              <div className="flex justify-between items-baseline">
                <span className="text-[11px] text-gray-600">{edu.institution || 'Institution'}</span>
        ...
        <Section title="Achievements">
          {resume.achievements.map((ach) => (
            <div key={ach.id} className="mb-0.5 last:mb-0">
              <div className="flex justify-between items-baseline">
                <span className="font-bold text-[11px]">{ach.title || 'Achievement'}</span>
                {ach.date && <span className="text-[11px] text-gray-500">{ach.date}</span>}
              </div>
              {ach.description && (
        ...
        <Section title="Publications">
          {resume.publications.map((pub) => (
            <div key={pub.id} className="mb-0.5 last:mb-0">
              <div className="flex justify-between items-baseline">
                <span className="font-bold text-[11px]">{pub.title || 'Publication'}</span>
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mb-1.5">
      <h2 className="text-[13px] font-bold uppercase tracking-wider text-black border-b border-gray-400 pb-0.5 mb-1">
        {title}
      </h2>
      {children}
    </div>
